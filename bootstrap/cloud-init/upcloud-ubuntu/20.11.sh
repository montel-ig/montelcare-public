...
install_iscsi() {
	...
	#$sh_c 'apt-get install -y nfs-common open-iscsi'
	$sh_c 'apt-get install -qy nfs-common open-iscsi xfsprogs && sudo systemctl enable iscsid && sudo systemctl start iscsid'
       # $sh_c 'rm -f /lib/systemd/system/nfs-common.service && systemctl daemon-reload && systemctl enable nfs-common && systemctl start nfs-common && systemctl enable open-iscsi'	
}
